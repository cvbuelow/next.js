...
const PREFETCH = process.env.__NEXT_PREFETCH;
const isModelPage = (pathname)=>pathname.includes('/designer/[userName]/3d-model/[modelIdentifier]') && !pathname.includes('project');
...
class Router {
   ...
   */ replace(url, as, options = {}) {
     ...
     */ let route = requestedRoute;
        try {
            ...
            if (routeProps.isModelPage && existingInfo || routeProps.shallow && existingInfo && this.route === route) {
                return existingInfo;
            }
   ...
   */ async prefetch(url, asPath = url, options = {}) {
     ...
     */ if (data?.effect.type === 'redirect-external') {
        ...
        const route = (0, _removetrailingslash.removeTrailingSlash)(pathname);
        const isPrefetchRoute = PREFETCH?.include && PREFETCH.include.includes(route) || PREFETCH?.exclude && !PREFETCH.exclude.includes(route);
        ...
        await Promise.all([
            this.pageLoader._isSsg(route).then((isSsg)=>{
                return isSsg || isPrefetchRoute ? fetchNextData({
                    dataHref: data?.json ? data?.dataHref : this.pageLoader.getDataHref({
                        href: isSsg ? url : (0, _formaturl.formatWithValidation)({
                            pathname: route,
                            query
                        }),
                        asPath: resolvedAs,
